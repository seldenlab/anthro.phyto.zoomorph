...
# size as a function of type?
fit.size.type <- procD.lm(size ~ type,
                data = gdf,
                print.progress = FALSE,
                iter = 9999)

# shape as a function of type?
...
# ANOVA: do type sizes differ?
anova(fit.size.type)

# ANOVA: do type shapes differ?
...
# subset landmark coordinates to produce mean shapes (group)
new.coords<-coords.subset(A = Y.gpa$coords,
                          group = qdata$group)
names(new.coords)
